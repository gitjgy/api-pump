...
public interface InventoryBookRepository extends JpaRepository<InventoryBookEntity, Long>,
	...
	// 期初(期初结存)：物料ID、最早时间
	@Query(value="SELECT last_qty FROM pump_inventory_book p WHERE p.mt_barcode=?1 ORDER BY p.cre_date ASC limit 0,1",nativeQuery=true)
	...
	// 库存(当期结存)：物料ID、最近时间
	@Query(value="SELECT cur_qty FROM pump_inventory_book p WHERE p.mt_barcode=?1 ORDER BY p.cre_date DESC limit 0,1",nativeQuery=true)
