...
//List<MaterialEntity> searchByBarcodeLike(String barCode);
	...
	@Transactional
	@Query(value = "UPDATE pump_material SET mt_in_total_quantity=?1,mt_in_remain_quantity=?2 WHERE mt_id=?3", nativeQuery = true)
	...
	//手动入库时，生成不重复的条码
	@Query(value="SELECT CONCAT(LEFT(mt_barcode,14),LPAD(RIGHT(mt_barcode,4)+1,4,0)) FROM pump_material WHERE LEFT(mt_barcode,14) = ?1 ORDER BY mt_barcode DESC LIMIT 0,1",nativeQuery = true)
	public String makeBarcode(String barcode);
